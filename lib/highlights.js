...
const findObjectByVideoId = videoId => {
	const matchId = videoId.includes('|||') ? videoId.split('|||')[0] : videoId;
...
const resolveStreamTarget = (object, videoId) => {
	if (!object?.videos?.length) {
		return { video: null, streamId: videoId };
	}

	const video = findVideoById(object, videoId);
	if (video) {
		return { video, streamId: videoId };
	}

	// Stremio catalog play requests streams with meta.id (match-level), not video.id.
	if (!videoId.includes('|||')) {
		const firstVideo = object.videos[0];
		return {
			video: firstVideo,
			streamId: sanitiseId([object.title, firstVideo.title])
		};
	}
...
const resolveYoutubeId = async (videoId, timeoutMs = 45000) => {
	...
	const { video, streamId } = resolveStreamTarget(object, videoId);
	if (video) {
	...
	while (!youtubeDict[streamId]) {
		if (Date.now() > deadline) {
			return null;
		}
		await sleep(100);
	}
	return youtubeDict[streamId];
};
...
const getStream = async videoId => {
	await loadData();
	const object = findObjectByVideoId(videoId);
	const { video, streamId } = object ? resolveStreamTarget(object, videoId) : { video: null, streamId: videoId };

	if (object && video) {
		registerVideo(object, video);
	}

	let embedPageUrl = scorebatEmbedDict[streamId];
	if (!embedPageUrl && video) {
		const scorebatId =
			video.scorebatId || parseScorebatVideoId(video.embed) || parseScorebatVideoId(parseEmbed(video.embed));
		if (scorebatId) {
			embedPageUrl = scorebatEmbedUrl(scorebatId);
			scorebatEmbedDict[streamId] = embedPageUrl;
		}
	}

	if (youtubeDict[streamId]) {
		const ytId = youtubeDict[streamId];
		return {
			youtubeId: ytId,
